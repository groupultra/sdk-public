...
from moobius.types import MessageContent, MessageBody, Button, ButtonClick, InputComponent, Payload, MenuItemClick, UpdateBody, UpdateItem, CopyBody, Character, ChannelInfo, CanvasItem, StyleItem, MenuItem, ClickArgument, ActionBody
...
class Moobius:
    ...
    def __init__(self, config: Optional[str|dict]=None, account_config: Optional[str|dict]=None, service_config: Optional[str|dict]=None, db_config: Optional[str|dict]=None, log_config: Optional[str|dict]=None):
        ...
        self.checkin_interval = self.config['service_config'].get('checkin_period', 90)
    ...
    async def create_new_service(self, description="Generated by MoobiusService"):
        """Creates a new service and sets self.client_id to it. Acceptions an optional description. Returns the service_id."""
    ...
    async def send_message(self, message, channel_id=None, sender=None, recipients=None, subtype=None, len_limit=None, file_display_name=None, context=None):
        """
        ...
        """
        message = types.normalize_message(message, channel_id, sender, recipients, subtype, len_limit, file_display_name, context)

        if message['content'].path:
            message['content'].path = await self.http_api.convert_to_url(message['content'].path)
    ...
    async def handle_received_payload(self, payload):
        ...
        if 'type' in payload_data:
            ...
            if payload.type == types.MESSAGE_DOWN:
                ...
                await self.on_refresh(ActionBody(**payload_body))
            else:
    ...
    async def on_action(self, action_data: dict):
        ...
        if subtype == types.JOIN:
            await self.on_join(ActionBody(**action_data))
        elif subtype == types.LEAVE_CHANNEL:
            await self.on_leave(ActionBody(**action_data))
            ...
            await self.on_refresh(ActionBody(**action_data))
        else:
    ...
    async def on_refresh(self, action: ActionBody):
        """
        This callback accepts a "Copy" request from the user. Returns None."""
        await self.do_member_sync(action.channel_id, action.sender)

    async def on_join(self, action: ActionBody):
        """
    ...
    async def on_leave(self, action: ActionBody):
        """
