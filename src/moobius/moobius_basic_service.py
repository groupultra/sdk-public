...
class MoobiusBasicService:
    ...
    def __init__(self, http_server_uri="", ws_server_uri="", service_id="", email="", password="", **kwargs):
        ...
        _ws_client = WSClient(ws_server_uri, on_connect=self.send_service_login, handle=self.handle_received_payload, horcrux=self.child_pipe)
    ...
    def start(self, bind_to_channels=None):
        ...
        loop.create_task(_ws_client.pipe_receive())
        
        process_forever = aioprocessing.AioProcess(target=main_loop.run_forever, args=())
    ...
    async def send(self, payload_type, payload_body):
        ...
        await _ws_client.send(payload_str)
