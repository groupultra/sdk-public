...
class MoobiusBasicService:
    def __init__(self, config_path="", http_server_uri="", ws_server_uri="", service_id="", email="", password="", create_new=True, **kwargs):
    ...
    async def start(self, bind_to_channels=None):
        ...
        if self.create_new:
            ...
            already_binded = False
            if bind_to_channels:
                for channel_id in bind_to_channels:
                    for service in service_list:
                        if channel_id in service["channel_ids"]:
                            self.service_id = service["service_id"]
                            log(f"Channel already binded.")
                            already_binded = True
                            break
            
            if not already_binded:
                self.service_id = self.http_api.create_service(description="Generated by MoobiusBasicService")
            log(f"Service ID: {self.service_id}, waiting for 5 seconds to bind to channels...")
            log("=================================================", print_to_console=True, color="magenta")
            log(f" Service ID:{self.service_id}", print_to_console=True, color="cyan")
            log("=================================================", print_to_console=True, color="magenta")
            try:
        ...
        if bind_to_channels:
            for channel_id in bind_to_channels:
                ...
                log(f"Bind to channel {channel_id}: {bind_info}")
        else:
            pass
